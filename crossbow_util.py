...
class Crossbow_Util:
            ...
            data = {
                ...
                "minecraft:attachable": {
                    "description": {
                        ...
                        "textures": {
                            "default": textures[0],
                            "crossbow_pulling_0": textures[1],
                            "crossbow_pulling_1": textures[2],
                            "crossbow_pulling_2": textures[3],
                            "charged": textures[4],
                            "firework": textures[5],
                            "enchanted": "textures/misc/charged_item_glint"
                        },
                        "geometry": {
                            "default": geometry[0],
                            "crossbow_pulling_0": geometry[1],
                            "crossbow_pulling_1": geometry[2],
                            "crossbow_pulling_2": geometry[3],
                            "charged": geometry[4],
                            "firework": geometry[5]
                        },
                        ...
                        "render_controllers": ["controller.render.crossbow_custom_3"]
                    }
                }
            }
    ...
    # ----------------------------------------------------
    # RENDER CONTROLLER 6 FRAMES
    # ----------------------------------------------------
    def rendercontrollers():
        file = "target/rp/render_controllers/crossbow_custom_3.render_controllers.json"
        ...
        with open(file, "w") as f:
            data = {
                "format_version": "1.10.0",
                "render_controllers": {
                    "controller.render.crossbow_custom_3": {
                        "arrays": {
                            "textures": {
    ...
    # ----------------------------------------------------
    # (ส่วนอื่น ๆ ทั้งหมดเหมือนเดิม ไม่ต้องแก้)
    # ----------------------------------------------------
    ...
    def animation():
        with open("target/rp/animations/crossbow_custom.animation.json","w") as f:
            data = {
    ...
    def is2Dcrossbow(file):
        ...
        if modelbone == [{"name":"geyser_custom","binding":"c.item_slot == 'head' ? 'head' : q.item_slot_to_bone_name(c.item_slot)","pivot":[0,8,0]},
    ...
    def acontroller(gmdllist):
        strlist = ",".join(gmdllist)
        file = "target/rp/animation_controllers/player.animation_controllers.json"
        os.makedirs(os.path.dirname(file), exist_ok=True)
        with open(file, "w") as f:
            data = {
                "format_version" : "1.10.0",
                        ...
                        "states" : {
                            ...
                            "third_person" : {
                                "animations" : [
                                    ...
                                    {
                                        "third_person_crossbow_equipped" : f"query.is_item_name_any('slot.weapon.mainhand', 0, 'minecraft:crossbow', {strlist}) && (q.is_using_item)"
                                    },
                                    {
