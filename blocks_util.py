...
def write_animated_cube():
    ...
    with open("target/rp/animations/cube.json", "w") as f:
        json.dump(data, f)
def write_geometry_cube():
    ...
    with open("target/rp/models/blocks/cube.json", "w") as f:
        json.dump(data, f)
def write_mapping_block(block: str):
    ...
    with open(f"target/geyser_block_{block}_mappings.json", "w") as f:
        json.dump(data, f, indent=4)
def regsister_block(block: str, gmdl: str, state: str, texture: str, block_material: str, geometry: str):
    with open(f"target/geyser_block_{block}_mappings.json", "r") as f:
        data = json.load(f)
    with open(f"target/geyser_block_{block}_mappings.json", "w") as f:
...
def get_geometry_block(model: str):
    namespace = model.split(":")[0]
    path = model.split(":")[1]
    geometry_file = glob.glob(f"target/rp/models/blocks/{namespace}/{path}.json")[0]
