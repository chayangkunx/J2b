...
DEBUG = os.getenv("DEBUG_SHIELD", "1") == "1"

if os.path.exists("assets/minecraft/models/item/shield.json"):
    if DEBUG:
        print("[shield] Loading OptiFine shield overrides…")
    with open("assets/minecraft/models/item/shield.json") as f:
        data = json.load(f)
    ...
    for m, p in zip(model, predicate):
        if m == "item/shield" or "custom_model_data" not in p:
            if DEBUG:
                print(f"[shield] Skip base entry: model={m}")
            continue
        try:
            fpath = (f"cache/shield/{p['custom_model_data']}.json")
            if DEBUG:
                print(f"[shield] Processing CMD={p['custom_model_data']} -> model={m}")
    ...
    if DEBUG:
        print("[shield] No OptiFine shield.json found, skipping")

for file in glob.glob("cache/shield/*.json"):
    with open(file, "r") as f:
        data = json.load(f)
    if DEBUG:
        print(f"[shield] Cache file {file} => {data}")
    if data["check"] == 2:
        animation = {}
        for i in ["default", "blocking"]:
            namespace = data[i].split(":")[0]
            path = data[i].split(":")[1]
            files = glob.glob(f"target/rp/attachables/{namespace}/{path}*.json")
            for fa in files:
        ...
        with open(saf, "w") as f:
            if DEBUG:
                print(f"[shield] Writing merged attachable {saf}")
...
def cleanup_cache():
    if DEBUG:
        print("[shield] Cleaning cache directory")
    if os.path.isdir("cache"):
        shutil.rmtree("cache", ignore_errors=True)

print("[shield] Completed OptiFine shield processing")
cleanup_cache()