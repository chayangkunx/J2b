...
try:
    with open("assets/minecraft/font/default.json", "r", encoding="utf-8") as f:
        data = json.load(f)
...
def cleanup():
    for folder in ["export", "images", "font"]:
        if os.path.isdir(folder):
            shutil.rmtree(folder, ignore_errors=True)
    if os.path.isfile("blankimg.png"):
        os.remove("blankimg.png")
